...
@dataclass
class TWAPTradeResult:
    """
    Result of a single time-weighted-average-price trend-following position.
...
@dataclass
class TWAPWeeklyExpiryResult:
    expiry_date: date
    atm_strike: int
    nifty_open: float
    ce_trades: list[TWAPTradeResult] = field(default_factory=list)
    pe_trades: list[TWAPTradeResult] = field(default_factory=list)

    @property
    def all_trades(self) -> list[TWAPTradeResult]:
        return self.ce_trades + self.pe_trades


@dataclass
class TWAPSymbolMetrics:
    symbol: str
    total_trades: int
    wins: int
    losses: int

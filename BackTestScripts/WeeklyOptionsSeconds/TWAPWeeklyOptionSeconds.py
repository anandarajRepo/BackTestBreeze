"""
Nifty 50 Weekly Options — Time-Weighted Average Price (TWAP) Backtest  (1-second data)
======================================================================================

Entry (long the option premium, CE & PE legs):
  buy when the option price makes a fresh cross above the session TWAP (the
  time-weighted average price, i.e. the cumulative arithmetic mean of the
  typical price across the day), is also trading above its trend EMA, and the
  EMA is rising. Unlike VWAP, the TWAP weights every bar equally by time and
  ignores traded volume, which makes it robust on thin / illiquid option chains.
    (optional momentum filter: entry bar must be >= MOMENTUM_FACTOR% above the
     previous bar's close — set to 0 to disable)
...
Exit:
  ...
  - Trend reversal (price closes below TWAP or the EMA crosses down)
...
from strategy.twap_option_strategy import TWAPOptionStrategy
...
# Optional momentum confirmation for entries. The entry bar's price must be at
# least MOMENTUM_FACTOR percent above the previous bar's close for the TWAP
# cross to be taken. 0 disables the momentum filter (pure TWAP+EMA signal).
MOMENTUM_FACTOR   = 0.0
...
if __name__ == "__main__":
    ...
    TWAPOptionStrategy.print_report(expiry_results)
